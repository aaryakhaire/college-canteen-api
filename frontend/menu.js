...
function enterEditMode(item) {
  ...
  formTitle.innerText = `Edit Menu Item #${item.id}`;
  formMsg.innerText = `Editing Item #${item.id}`;
}
...
function exitEditMode() {
  ...
  formTitle.innerText = "Add Menu Item";
  formMsg.innerText = "";
}
...
menuForm.addEventListener("submit", async (e) => {
  ...
  if (editId) {
    formMsg.innerText = "Saving changes...";
    const res = await fetch(`${API_BASE}/menu/${editId}`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(payload),
    });
    ...
    if (!res.ok) {
      formMsg.innerText = data.error || "Failed to update item";
      return;
    }

    formMsg.innerText = "Updated successfully ✅";
    exitEditMode();
    await fetchMenu();
    return;
  }
